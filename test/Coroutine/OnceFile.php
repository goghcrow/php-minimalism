...
use Zan\Framework\Foundation\Coroutine\Async;
...
class OnceFile implements Async
{
    ...
    public function readHandle(){
        ...
        $this->content = "";
        swoole_async_read($this->filename, function($filename, $content) {
            if ($content === self::EOF) {
                $cb = $this->callback;
    ...
    public function writeHandle(){
        ...
        if ($content === false) {
            $cb = $this->callback;
            $cb($this->filename, false);
            return;
        }

        swoole_async_write($this->filename, $content, $this->offset, function($filename, $size) {
