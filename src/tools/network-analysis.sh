...
# 1、实时监控任意网卡的流量
# 2、统计10秒内平均流量
# 3、统计每个端口在10秒内的平均流量，基于客户端和服务端端口统计。
# 可以看出哪些端口占流量比较大，对于web服务器，一般是80端口。
# 其它端口受到攻击时，也有可能其它端口流量比较大。所以此功能可以帮助我们端口流量是否正常。
# 4、统计在10s内占用带宽最大的前10个ip。此项功能可以帮助我们来查出是否有恶意占用带宽的ip。
# 5、统计连接状态。此项功能可以让我们看出哪些连接状态比较大。如果SYN-RECV状态比较多的话，有可以受到半连接攻击。
# 如果ESTABLISED非常大，但通过日志发现没有那么多请求，或者通过tcpdump发现大量ip只建立连接不请求数据的话，
# 可能是受到了全连接攻击，这时候如果你使用的是nginx服务器，可以在配置文件增加listen 80 deferred来防止。
...
# 使用也很简单，直接运行，我们根据提示选择数字即可。
# sh network-analysis.sh
# 1) real time traffic.
# 2) traffic and connection overview.

# please input your select(ie 1):

# 如果选择1，那么会显示实时网卡传输速率。如果选择2，会让我们等待10s，然后给出一个统计数据。