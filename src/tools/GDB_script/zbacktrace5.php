...
if (!isset($argv[1]) || !file_exists("/proc/$argv[1]")) {
    exit("Usage: sudo $argv[0] <PID>\n");
}

$pid = $argv[1];

$tmpFile = tempnam(sys_get_temp_dir(), ".__");
file_put_contents($tmpFile, $gdbInit);

$php = rtrim(`which php`);
echo `gdb --batch -nx $php $pid -ex "source $tmpFile"`; // ? 2>/dev/null
unlink($tmpFile);