...
foreach (["host", "port", "user", "pwd", "db"] as $arg) {
    if (!isset($_GET[$arg])) {
        exit("Usage:?host=&port=&user=&pwd=&db=");
    }
}
...
ORDER BY a.CREATE_TIME ASC, a.ORDINAL_POSITION ASC;
SQL;

$dsn = "mysql:dbname=$dbname;charset=utf8;host=$dbhost;port=$dbport";
try {
